...
Given('UI - I enter {string} employes', async function (employe) {
   await fourthStepPage.enterInputForEmployes(employe)

});

When('UI - I click {string} do all of your employes have email address', async function (answer) {
    await fourthStepPage.clickDoAllYourEmoloyesButton(answer);
});

When('UI - I click confirm button', async function () {
    await fourthStepPage.clickConfirmButton();
});


When('UI - I choose {string} type of upload and uplaod {string} file', async function (typeOfUpload, file) {
    switch(typeOfUpload) {
        case 'CERTIFICATION':
...
When('UI - I expect that confirm upload button is not clickable', async function () {
...
When('UI - I expect that analyst message is {string}', async function (msg) {
