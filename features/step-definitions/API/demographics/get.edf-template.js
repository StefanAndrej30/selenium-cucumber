...
Then('I download {string} file', async function (file) {
  await downloadFile(`/api/en-US/Project/${this.projectId}/Survey/${this.surveyId}/EDFTemplate/File?bearer_token=${authorize.getToken('userToken')}&h_gptw_client_id=${this.clientId}`, `../../tmp/${file}`);
});
...
Then('Check file:', async function (docString) { // with json
  const arr = [];
  const sheetName = await globals.getAllSheets('../../tmp/YYY-template.xlsx');

  for (let i = 0; i < sheetName.length; i += 1) {
    const temp = await xlsx.utils.sheet_to_json(await globals.readXlsxFile('../../tmp/YYY-template.xlsx', sheetName[i]));
    temp.forEach((res) => {
      arr.push(res);
    });
  }
